/**
 * Netlify Scheduled Function: crawl-contests-cron  (자동 수집 전용)
 * netlify.toml 의 schedule 로만 실행됨. URL 직접 호출은 Netlify가 403 으로 막음.
 * 수동 수집은 crawl-contests 함수를 사용.
 *
 * 원본
...
exports.handler = async () => {
  try {
    ...
    console.log('[crawl-cron]', res.report.join(' | '),
                `→ ${res.items.length}건 저장 (관련 ${rel})`);
    return { statusCode: 200, body: JSON.stringify({ ok: true, saved: res.items.length }) };
  } catch (e) {
    console.error('[crawl-cron] error:', e);
